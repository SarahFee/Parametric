...
try:
    ...
    def fetch_hdx_data(api_key, country_code="SDN", datasets=None): return None
...
#         logger.debug("Using default fallback for IATI organization definitions.")
#         return [
#             {"name": "World-Vision", "org_type": "NGO", "initial_capital": 3000000, "total_budget": 8000000},
#             {"name": "UNHCR", "org_type": "UN Agency", "initial_capital": 6000000, "total_budget": 15000000},
#             {"name": "IOM", "org_type": "Hybrid", "initial_capital": 4500000, "total_budget": 10000000}
#         ]
...
#         # Check if it's time to donate based on frequency (add some randomness)
#         # Simple check: Donate roughly every 'donation_frequency' steps
#         # More realistic: probability increases as steps_since_last_donation grows
#         prob_donate = self.steps_since_last_donation / (self.donation_frequency * 2)  # Example probability ramp
#         if random.random() < prob_donate or self.steps_since_last_donation >= self.donation_frequency * 1.5:  # Ensure donation eventually happens
#             try:
#                 # Determine donation amount with some variability (e.g., log-normal or gamma dist?)
#                 # Using uniform for simplicity now
#                 donation_amount = max(1000, self.avg_donation * random.uniform(0.7, 1.3))  # Ensure min amount
...
#             # Check for Emergency Event
#             base_emergency_prob = getattr(self.model, 'emergency_probability', 0.05)  # Get from model
#             effective_emergency_prob = base_emergency_prob * self.emergency_probability_modifier
#             if random.random() < effective_emergency_prob:
#                 emergency_cost = self.emergency_impact * self.total_budget
#                 self.balance -= emergency_cost
...
#             # Check for Security Event
#             base_security_risk = getattr(self.model, 'security_risk_factor', 0.2)  # Get from model
#             effective_security_risk = base_security_risk * self.security_risk_modifier * monthly_security_modifier
#             # Set security impact - higher impact for UN agencies, make configurable?
#             security_impact_factor = 0.07 if self.org_type == "UN Agency" else 0.05
...
#         self.emergency_probability = 0.05  # 5% base probability
#         self.security_risk_factor = 0.2    # 20% base probability
...
#         # If no IATI defs or error, fall back to default definitions
#         if not org_definitions:
#             org_definitions = [
#                 {"name": "World-Vision", "org_type": "NGO", "initial_capital": 3000000, "total_budget": 8000000},
#                 {"name": "UNHCR", "org_type": "UN Agency", "initial_capital": 6000000, "total_budget": 15000000},
#                 {"name": "IOM", "org_type": "Hybrid", "initial_capital": 4500000, "total_budget": 10000000}
#             ]
#             logger.info("Using default organization definitions.")
...
#         # Track individual organization impacts
#         for org in self.orgs:
#             impact = org.balance - org.initial_capital
#             self.org_impact_history[org.name].append(impact)
